...
- **20 Playwright tests** (`tests/features/test_interest_income.py`, `tests/features/test_storage_import.py`, `tests/integration/test_interest_income_workflow.py`) — engine math, compounding, override feedback, last-day posting, projection-only persistence, negative/zero/sub-cent skips, accounts UI CRUD, reports integration, import sanitization (clamping −5 → 0, 200 → 100, junk → 0, round-trip), and an end-to-end workflow
