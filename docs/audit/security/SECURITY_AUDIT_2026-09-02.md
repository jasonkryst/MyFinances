...
**Investigated 2026-09-03, fix rejected:** the obvious fix — setting `NODE_ENV: production` directly in `docker-compose.yml`'s `server` service — was implemented and then reverted after discovering it conflicts with `DEPLOYMENT.md`'s "HTTPS Requirement" section, which deliberately documents the *opposite* sequencing: `NODE_ENV=production` must be set **only after** HTTPS is terminated in front of the stack, because `auth.js`'s `SECURE` flag has no protocol-detection fallback — setting it while still testing over plain HTTP causes the browser to silently drop the session/CSRF cookies (login breaks, with no error surfaced to the user). That `DEPLOYMENT.md` guidance was itself written deliberately as part of the 2026-09-02 documentation-audit fix. Baking the setting into `docker-compose.yml` unconditionally would fix this Medium finding for HTTPS-terminated deployments at the cost of breaking login for the plain-HTTP local-testing path the docs explicitly support — a functional regression traded for a security hardening, not a clean fix. Left open pending a real solution (e.g., a dedicated env var an operator sets only once HTTPS is confirmed working, or `X-Forwarded-Proto`-based detection now that `trust proxy` is being considered for M1) rather than accepting the tradeoff.
