...
**RESOLVED 2026-09-03:** `stryker.config.mjs`'s `sanitizers.js` ranges are now `5-58`, `72-93`, `95-125`, `126-147` — the last three matching this section's recommendation exactly (`95-125` merges the two new functions' `95-124` with a +1 fix at the boundary); `5-58` extends the original `5-54` past its own pre-existing truncation of `sanitizeIncome`'s last few lines, unrelated to the `aa6ead3` shift this section describes. `tests/unit/sanitizers.test.js` gained 8 tests covering `sanitizeLedgerOverrides`/`sanitizeLedgerClearedTransactions` (non-object input, dropped-entry cases, a well-formed round-trip, and the `updatedAt` default-to-now behavior). `thresholds` re-derived from a fresh run: 46.93% → `high: 51, low: 46, break: 36`.
