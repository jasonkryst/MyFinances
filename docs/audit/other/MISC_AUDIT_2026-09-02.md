...
- `qs` (moderate — array-limit bypass, DoS via attacker-controlled `isBuffer`) via `body-parser` → `express` 4.22.2. This one **is** in the request-handling path (`express.json()`/`express.urlencoded()` parse the `qs` query-string library on every request) and is fixable non-breaking via plain `npm audit fix` (no `--force`/major bump needed) — this is the one item in the whole audit worth prioritizing over the others, since it sits directly in front of authenticated API traffic.
...
**[Low] `.plans/MIGRATION_PLAN.md` is stale planning scratch, not active work**
