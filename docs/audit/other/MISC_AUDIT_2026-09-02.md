...
**[Low] Tracked: [#157](https://github.com/jasonkryst/MyFinances/issues/157). RESOLVED v5.4.4 (2026-09-14)** — Both `.github/workflows/codeql.yml` and `.github/workflows/trivy.yml` now pin `github/codeql-action` at `@v4` throughout. A regression test `test_codeql_action_version_consistent` in `tests/security/test_static_scan.py` asserts the major version is consistent across both files so drift cannot recur silently.
