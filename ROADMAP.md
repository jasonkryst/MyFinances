...
- ~~**Formal storage-schema migration framework**~~ ✅ **Delivered August 23, 2026 (#53, Phase 1)** — the new self-hosted Postgres backend (`server/migrations/`, via `node-pg-migrate`) has an explicit migration pipeline; the localStorage JSON blob's sanitizer-based migration is unaffected and continues to serve local-only users, who remain fully supported.
